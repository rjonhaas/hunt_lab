#!/usr/bin/env python3
"""
Generates and imports the Hunt Lab Threat Hunt Report Template into Kibana.
Target: Kibana 8.19.x at http://192.168.56.10:5601
"""

import json, sys

KIBANA_URL = "http://192.168.56.10:5601"
DATA_VIEW_ID = "hunt-lab-logs-data-view"
DASH_ID = "hl-threat-hunt-report-template"

# ── Markdown content ──────────────────────────────────────────────────────────

HUNT_SUMMARY_MD = """## Hunt Summary

| Field | Value |
|-------|-------|
| **Scenario** | _(enter scenario name / ATT&CK technique)_ |
| **Analyst** | _(analyst name)_ |
| **Date Range** | _(start date) → (end date)_ |
| **Hypothesis** | _(what adversary behaviour are you looking for?)_ |
| **Scope** | _(affected hosts / users / systems)_ |
| **Hunt ID** | HL-YYYY-NNN |

> **Instructions:** Duplicate this dashboard before filling it in.
> Use the time picker above to constrain all panels to the hunt window.
"""

SECTION2_HDR = "## Key Metrics\n\n*Counts reflect data within the selected time range.*"

SECTION3_HDR = """## Evidence Panels

*Filter panels by adjusting the KQL bar or time picker above.
All panels use common ECS fields — add index-specific filters as needed.*"""

FINDINGS_MD = """## Findings

### Observations
_(What did you observe during the hunt? List significant events, patterns, or anomalies.)_

- Observation 1:
- Observation 2:
- Observation 3:

### Suspicious Indicators
| Indicator | Type | Context |
|-----------|------|---------|
| _(e.g. 192.168.1.99)_ | IP | Outbound to rare destination |
| _(e.g. lsass.exe dump)_ | Process | Credential access pattern |
| | | |
"""

ASSESSMENT_MD = """## Assessment

### Verdict
- [ ] True Positive — confirmed malicious activity
- [ ] True Positive — suspicious, requires escalation
- [ ] Benign Positive — expected behaviour
- [ ] False Positive — rule / data quality issue
- [ ] Inconclusive — insufficient data

### Why
_(Explain the reasoning behind the verdict.
Reference specific evidence panels, IOCs, or ATT&CK techniques.)_

### Recommended Actions
1. _(e.g. Isolate host XYZ)_
2. _(e.g. Reset credentials for user ABC)_
3. _(e.g. Create detection rule for T1059.001)_
"""

DETECTION_MD = """## Detection Opportunities

*Use this section to capture follow-on detection engineering ideas discovered during the hunt.*

| Opportunity | ATT&CK Technique | Priority | Notes |
|------------|-----------------|----------|-------|
| _(e.g. Alert on encoded PowerShell args)_ | T1059.001 | High | _(implementation notes)_ |
| _(e.g. Detect rare parent-child process)_ | T1055 | Medium | |
| | | | |

### Sigma / EQL Sketch
```
# Paste a Sigma rule or EQL query sketch here
```
"""

# ── Helpers ───────────────────────────────────────────────────────────────────

def mk_markdown_panel(i, x, y, w, h, markdown, title=""):
    return {
        "type": "visualization",
        "gridData": {"x": x, "y": y, "w": w, "h": h, "i": i},
        "panelIndex": i,
        "title": title,
        "embeddableConfig": {
            "savedVis": {
                "title": title,
                "description": "",
                "type": "markdown",
                "params": {
                    "fontSize": 12,
                    "markdown": markdown,
                    "openLinksInNewTab": False,
                },
                "uiState": {},
                "data": {
                    "aggs": [],
                    "searchSource": {
                        "query": {"language": "kuery", "query": ""},
                        "filter": [],
                    },
                },
            },
            "enhancements": {},
        },
    }


def mk_lens_panel(i, x, y, w, h, ref_name, title=""):
    return {
        "type": "lens",
        "gridData": {"x": x, "y": y, "w": w, "h": h, "i": i},
        "panelIndex": i,
        "title": title,
        "panelRefName": ref_name,
        "embeddableConfig": {"enhancements": {}},
    }


def mk_metric_lens(obj_id, title, operation, source_field, color="#1BA9F5"):
    col_id = "col-metric"
    layer_id = "layer-1"
    return {
        "type": "lens",
        "id": obj_id,
        "attributes": {
            "title": title,
            "description": "",
            "visualizationType": "lnsMetric",
            "state": {
                "datasourceStates": {
                    "formBased": {
                        "layers": {
                            layer_id: {
                                "columns": {
                                    col_id: {
                                        "label": title,
                                        "dataType": "number",
                                        "operationType": operation,
                                        "isBucketed": False,
                                        "scale": "ratio",
                                        "sourceField": source_field,
                                        "params": {"emptyAsNull": False},
                                    }
                                },
                                "columnOrder": [col_id],
                                "incompleteColumns": {},
                                "sampling": 1,
                            }
                        }
                    }
                },
                "visualization": {
                    "layerId": layer_id,
                    "layerType": "data",
                    "metricAccessor": col_id,
                    "color": color,
                },
                "query": {"language": "kuery", "query": ""},
                "filters": [],
                "internalReferences": [],
                "adHocDataViews": {},
            },
        },
        "references": [
            {
                "type": "index-pattern",
                "id": DATA_VIEW_ID,
                "name": f"indexpattern-datasource-layer-{layer_id}",
            }
        ],
    }


def mk_datatable_lens(obj_id, title, bucket_cols, kql_filter=""):
    """bucket_cols: list of (col_id, label, source_field)"""
    layer_id = "layer-1"
    count_col = "col-count"

    columns = {}
    col_order = []

    for col_id, label, source_field in bucket_cols:
        columns[col_id] = {
            "label": label,
            "dataType": "string",
            "operationType": "terms",
            "isBucketed": True,
            "scale": "ordinal",
            "sourceField": source_field,
            "params": {
                "size": 20,
                "orderBy": {"type": "column", "columnId": count_col},
                "orderDirection": "desc",
                "otherBucket": False,
                "missingBucket": False,
                "parentFormat": {"id": "terms"},
            },
        }
        col_order.append(col_id)

    columns[count_col] = {
        "label": "Count",
        "dataType": "number",
        "operationType": "count",
        "isBucketed": False,
        "scale": "ratio",
        "sourceField": "___records___",
        "params": {"emptyAsNull": False},
    }
    col_order.append(count_col)

    viz_columns = [{"columnId": cid, "hidden": False} for cid in col_order]

    return {
        "type": "lens",
        "id": obj_id,
        "attributes": {
            "title": title,
            "description": "",
            "visualizationType": "lnsDatatable",
            "state": {
                "datasourceStates": {
                    "formBased": {
                        "layers": {
                            layer_id: {
                                "columns": columns,
                                "columnOrder": col_order,
                                "incompleteColumns": {},
                                "sampling": 1,
                            }
                        }
                    }
                },
                "visualization": {
                    "layerId": layer_id,
                    "layerType": "data",
                    "columns": viz_columns,
                    "sorting": {"columnId": count_col, "direction": "desc"},
                },
                "query": {"language": "kuery", "query": kql_filter},
                "filters": [],
                "internalReferences": [],
                "adHocDataViews": {},
            },
        },
        "references": [
            {
                "type": "index-pattern",
                "id": DATA_VIEW_ID,
                "name": f"indexpattern-datasource-layer-{layer_id}",
            }
        ],
    }


def mk_area_chart_lens(obj_id, title):
    layer_id = "layer-1"
    time_col = "col-time"
    count_col = "col-count"

    return {
        "type": "lens",
        "id": obj_id,
        "attributes": {
            "title": title,
            "description": "",
            "visualizationType": "lnsXY",
            "state": {
                "datasourceStates": {
                    "formBased": {
                        "layers": {
                            layer_id: {
                                "columns": {
                                    time_col: {
                                        "label": "@timestamp",
                                        "dataType": "date",
                                        "operationType": "date_histogram",
                                        "sourceField": "@timestamp",
                                        "isBucketed": True,
                                        "scale": "interval",
                                        "params": {
                                            "interval": "auto",
                                            "includeEmptyRows": True,
                                            "dropPartials": False,
                                        },
                                    },
                                    count_col: {
                                        "label": "Events",
                                        "dataType": "number",
                                        "operationType": "count",
                                        "isBucketed": False,
                                        "scale": "ratio",
                                        "sourceField": "___records___",
                                        "params": {"emptyAsNull": False},
                                    },
                                },
                                "columnOrder": [time_col, count_col],
                                "incompleteColumns": {},
                                "sampling": 1,
                            }
                        }
                    }
                },
                "visualization": {
                    "legend": {"isVisible": True, "position": "right"},
                    "valueLabels": "hide",
                    "fittingFunction": "None",
                    "axisTitlesVisibilitySettings": {
                        "x": True,
                        "yLeft": True,
                        "yRight": True,
                    },
                    "tickLabelsVisibilitySettings": {
                        "x": True,
                        "yLeft": True,
                        "yRight": True,
                    },
                    "gridlinesVisibilitySettings": {
                        "x": True,
                        "yLeft": True,
                        "yRight": True,
                    },
                    "preferredSeriesType": "area",
                    "layers": [
                        {
                            "layerId": layer_id,
                            "accessors": [count_col],
                            "layerType": "data",
                            "seriesType": "area",
                            "xAccessor": time_col,
                        }
                    ],
                },
                "query": {"language": "kuery", "query": ""},
                "filters": [],
                "internalReferences": [],
                "adHocDataViews": {},
            },
        },
        "references": [
            {
                "type": "index-pattern",
                "id": DATA_VIEW_ID,
                "name": f"indexpattern-datasource-layer-{layer_id}",
            }
        ],
    }


# ── Lens objects ──────────────────────────────────────────────────────────────

lens_objects = [
    mk_metric_lens(
        "hl-metric-total-events",
        "Total Events",
        "count",
        "___records___",
        "#1BA9F5",
    ),
    mk_metric_lens(
        "hl-metric-hosts",
        "Unique Hosts",
        "unique_count",
        "host.name",
        "#00BFB3",
    ),
    mk_metric_lens(
        "hl-metric-users",
        "Unique Users",
        "unique_count",
        "user.name",
        "#F04E98",
    ),
    mk_metric_lens(
        "hl-metric-event-cats",
        "Event Categories",
        "unique_count",
        "event.category",
        "#FEC514",
    ),
    mk_datatable_lens(
        "hl-table-processes",
        "HL - Process Execution Review",
        [
            ("col-host", "Host", "host.name"),
            ("col-user", "User", "user.name"),
            ("col-process", "Process", "process.name"),
        ],
        kql_filter="event.category: process",
    ),
    mk_datatable_lens(
        "hl-table-auth",
        "HL - Authentication Activity",
        [
            ("col-user", "User", "user.name"),
            ("col-host", "Host", "host.name"),
            ("col-outcome", "Outcome", "event.outcome"),
        ],
        kql_filter="event.category: authentication",
    ),
    mk_area_chart_lens("hl-chart-timeline", "HL - Activity Over Time"),
    mk_datatable_lens(
        "hl-table-network",
        "HL - Network Destinations",
        [
            ("col-src-host", "Source Host", "host.name"),
            ("col-dst-ip", "Dest IP", "destination.ip"),
            ("col-dst-domain", "Dest Domain", "destination.domain"),
        ],
        kql_filter="event.category: network",
    ),
]

# ── Dashboard panels ──────────────────────────────────────────────────────────

panels = []
refs = []
ref_idx = 0

def add_lens_panel(i, x, y, w, h, lens_id, title=""):
    ref_name = f"panel_{ref_idx}"
    panels.append(mk_lens_panel(i, x, y, w, h, ref_name, title))
    refs.append({"type": "lens", "id": lens_id, "name": ref_name})
    return ref_name

# Section 1 header
panels.append(mk_markdown_panel("md-s1-hdr", 0, 0, 48, 3,
    "# Threat Hunt Report Template\n*Hunt Lab — Interactive Threat Hunt Report*\n\n---"))

# Hunt summary
panels.append(mk_markdown_panel("md-s1-body", 0, 3, 48, 14, HUNT_SUMMARY_MD, "Hunt Summary"))

# Section 2 header
panels.append(mk_markdown_panel("md-s2-hdr", 0, 17, 48, 3, SECTION2_HDR))

# Metrics row (y=20)
ref_idx = 0
panels.append(mk_lens_panel("p-metric-1", 0, 20, 12, 9, "panel_0", "Total Events"))
refs.append({"type": "lens", "id": "hl-metric-total-events", "name": "panel_0"})
panels.append(mk_lens_panel("p-metric-2", 12, 20, 12, 9, "panel_1", "Unique Hosts"))
refs.append({"type": "lens", "id": "hl-metric-hosts", "name": "panel_1"})
panels.append(mk_lens_panel("p-metric-3", 24, 20, 12, 9, "panel_2", "Unique Users"))
refs.append({"type": "lens", "id": "hl-metric-users", "name": "panel_2"})
panels.append(mk_lens_panel("p-metric-4", 36, 20, 12, 9, "panel_3", "Event Categories"))
refs.append({"type": "lens", "id": "hl-metric-event-cats", "name": "panel_3"})

# Section 3 header
panels.append(mk_markdown_panel("md-s3-hdr", 0, 29, 48, 4, SECTION3_HDR))

# Evidence panels (y=33)
panels.append(mk_lens_panel("p-procs", 0, 33, 24, 17, "panel_4", "Process Execution Review"))
refs.append({"type": "lens", "id": "hl-table-processes", "name": "panel_4"})
panels.append(mk_lens_panel("p-auth", 24, 33, 24, 17, "panel_5", "Authentication Activity"))
refs.append({"type": "lens", "id": "hl-table-auth", "name": "panel_5"})

panels.append(mk_lens_panel("p-timeline", 0, 50, 48, 13, "panel_6", "Activity Over Time"))
refs.append({"type": "lens", "id": "hl-chart-timeline", "name": "panel_6"})

panels.append(mk_lens_panel("p-network", 0, 63, 48, 17, "panel_7", "Network Destinations"))
refs.append({"type": "lens", "id": "hl-table-network", "name": "panel_7"})

# Section 4 — Findings
panels.append(mk_markdown_panel("md-s4", 0, 80, 48, 16, FINDINGS_MD, "Findings"))

# Section 5 — Assessment
panels.append(mk_markdown_panel("md-s5", 0, 96, 48, 16, ASSESSMENT_MD, "Assessment"))

# Section 6 — Detection Opportunities
panels.append(mk_markdown_panel("md-s6", 0, 112, 48, 14, DETECTION_MD, "Detection Opportunities"))

# ── Dashboard object ──────────────────────────────────────────────────────────

dashboard = {
    "type": "dashboard",
    "id": DASH_ID,
    "attributes": {
        "title": "HL - Threat Hunt Report Template",
        "description": "Hunt Lab — Interactive Threat Hunt Report Template. Duplicate before use.",
        "panelsJSON": json.dumps(panels, separators=(",", ":")),
        "optionsJSON": json.dumps({
            "useMargins": True,
            "syncColors": False,
            "syncCursor": True,
            "syncTooltips": False,
            "hidePanelTitles": False,
        }),
        "timeRestore": False,
        "kibanaSavedObjectMeta": {
            "searchSourceJSON": json.dumps({
                "query": {"language": "kuery", "query": ""},
                "filter": [],
            })
        },
        "version": 1,
        "hits": 0,
    },
    "references": refs,
}

# ── Data view object ──────────────────────────────────────────────────────────

data_view = {
    "type": "index-pattern",
    "id": DATA_VIEW_ID,
    "attributes": {
        "name": "Hunt Lab Logs",
        "title": "logs-*",
        "timeFieldName": "@timestamp",
    },
    "references": [],
    "managed": False,
}

# ── Assemble NDJSON ───────────────────────────────────────────────────────────

all_objects = [data_view] + lens_objects + [dashboard]

output_path = "kibana/hunt_report_template.ndjson"
with open(output_path, "w", encoding="utf-8") as f:
    for obj in all_objects:
        f.write(json.dumps(obj, separators=(",", ":")) + "\n")

print(f"Written {len(all_objects)} objects to {output_path}")
print("Objects:")
for o in all_objects:
    print(f"  {o['type']:20s}  {o['id']}")
